...
function toggleStatusTooltip(): void {
  isStatusTooltipVisible.value = !isStatusTooltipVisible.value
}
...
<template>
  <header class="sticky top-0 z-20 w-full border-b border-slate-200 bg-white px-6 py-3 shadow-sm">
    <div class="flex w-full items-center justify-between gap-6">
      <div class="flex min-w-0 items-center gap-5">
        ...
        <div class="flex items-center gap-3">
          <div class="w-32 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <p class="text-[11px] font-medium uppercase tracking-wide text-slate-500">
              Porta
            </p>

            <p class="mt-1 truncate font-mono text-sm font-semibold text-slate-950">
              {{ port }}
            </p>
          </div>

          <div class="w-48 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <p class="text-[11px] font-medium uppercase tracking-wide text-slate-500">
              Token
            </p>
        ...
        <div class="relative flex items-center">
          <div class="flex h-8 w-8 items-center justify-center" :title="statusLabel" :aria-label="statusLabel"
            role="status" @mouseenter="showStatusTooltip" @mouseleave="hideStatusTooltip">
            <span class="h-3 w-3 rounded-full shadow-md" :class="statusDotClass" />
          </div>

          <div v-if="isStatusTooltipVisible"
      ...
      <button type="button"
        ...
        :disabled="isUnpairing" @click="unpair">
        <i v-if="!isUnpairing" class="pi pi-unlink text-xs" />
